#!/bin/bash
# simple-obfs(HTTP 伪装)交叉验证:NTR obfs 传输 <-> {mihomo, sing-box}。叠法 [obfs, shadowsocks]。
# obfs 首包裹假 websocket-upgrade GET(数据在体)+ 之后裸流。NTR obfs 客户端/服务端 ↔ mihomo 双向 + sing-box→NTR。
# mihomo:proxy plugin=obfs / listener simple-obfs.enable;sing-box:plugin=obfs-local(仅客户端)。
set -u
NET=ix-ob; PFX=ixobt-; D=/tmp/ntr-interop
...
log-level: warning
listeners:
  - {name: ss-in, type: shadowsocks, listen: 0.0.0.0, port: 10000, password: "$PW", cipher: $M, simple-obfs: {enable: true, mode: http}}
Y
}
mihomo_cli(){ cat > "$1" <<Y
log-level: warning
...
allow-lan: true
proxies:
  - {name: o, type: ss, server: ${PFX}s, port: 10000, cipher: $M, password: "$PW", plugin: obfs, plugin-opts: {mode: http, host: $HOST}}
rules: ["MATCH,o"]
Y
}
singbox_cli(){ cat > "$1" <<J
{"log":{"level":"warn"},"inbounds":[{"type":"mixed","listen":"::","listen_port":1080}],"outbounds":[{"type":"shadowsocks","server":"${PFX}s","server_port":10000,"method":"$M","password":"$PW","plugin":"obfs-local","plugin_opts":"obfs=http;obfs-host=$HOST"}]}
J
}
