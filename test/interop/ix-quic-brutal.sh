...
inbounds:
  - listen: 0.0.0.0:10000
    layers: [{type: quic, congestion: brutal, up-mbps: 100, down-mbps: 100}, {type: vless}]
    users: [{uuid: "$UUID"}]
    outbound: direct
...
outbounds:
  - {name: up, type: proxy, server: "${PFX}s:10000", secret: "$UUID", layers: [{type: quic, sni: example.com, insecure: true, congestion: brutal, up-mbps: 100, down-mbps: 100}, {type: vless}]}
Y
