...
docker run -d --name cip-srv --network $NET -e NTR_DEBUG=1 -v ${NTRL3_BIN:-/tmp/ntr-interop/ntr-l3}:/ntr:ro -v $D/cip-srv.yaml:/c.yaml:ro -v $D/cert.pem:/cert.pem:ro -v $D/key.pem:/key.pem:ro alpine /ntr -config /c.yaml >/dev/null 2>&1
# 客户端:socks 入站 → connect-ip 出站
printf 'inbounds:\n  - {listen: 0.0.0.0:1080, layers: [{type: socks}], outbound: up}\noutbounds:\n  - name: up\n    type: connect-ip\n    server: "cip-srv:8443"\n    sni: %s\n    insecure: true\n    local-address: ["10.9.0.2/32"]\n' "$DEST" > $D/cip-cli.yaml
docker run -d --name cip-cli --network $NET -e NTR_DEBUG=1 -v ${NTRL3_BIN:-/tmp/ntr-interop/ntr-l3}:/ntr:ro -v $D/cip-cli.yaml:/c.yaml:ro alpine /ntr -config /c.yaml >/dev/null 2>&1
sleep 7
