...
const (
	CmdPing         = 0
	CmdConnect      = 1
	CmdConnectReuse = 5 // v2 的 CONNECT 也用它(ConnectV2)
	CmdUDP          = 6
)
...
func parseCommand(b []byte) (*command, int, error) {
	...
	if b[0] != 1 {
		return nil, 0, fmt.Errorf("snellv123: bad version %d", b[0])
	}
	cmd := b[1]
	if cmd == CmdPing {
	...
	if cmd&0xFB != 1 && cmd != CmdUDP {
		return nil, 0, fmt.Errorf("snellv123: unknown command %d", cmd)
	}
	if len(b) < 3 {
		return nil, 0, nil
	}
...
// Client 是 Snell v1/v2/v3 客户端。ChaCha=true(v1)否则 AES-128(v2/v3);ConnectCmd=命令字节
// (v2=CmdConnectReuse,v1/v3=CmdConnect)。
type Client struct {
	PSK        []byte
	ChaCha     bool
	ConnectCmd byte
...
// DialTCPOver 在已建立的 conn 上跑 CONNECT 握手,返回承载中继 payload 的 net.Conn。
func (c *Client) DialTCPOver(conn net.Conn, host string, port uint16, initial []byte) (net.Conn, error) {
	cc := &clientConn{conn: conn, br: bufio.NewReader(conn),
		send: NewSenderSS(c.PSK, c.ChaCha), recv: NewReceiverSS(c.PSK, c.ChaCha)}
	connectCmd := c.ConnectCmd
	if connectCmd == 0 {
		connectCmd = CmdConnect
	}
	cmd := make([]byte, 0, 6+len(host)+len(initial))
	cmd = append(cmd, 1, connectCmd, 0, byte(len(host))) // ver=1, CONNECT, idLen=0, hostLen
	cmd = append(cmd, host...)
	var pb [2]byte
	...
	for len(c.rbuf) == 0 {
		...
		if !c.gotStatus {
			...
			if payload[0] == 0x02 { // 0x02 = 服务端错误 [type][len][msg]
				...
				return 0, fmt.Errorf("snellv123: server error: %s", msg)
			}
...
// Server 是 Snell v1/v2/v3 服务端(单端口 PSK)。ChaCha 由版本定(v1=true)。
type Server struct {
	PSK              []byte
	ChaCha           bool
...
// AcceptResult 是服务端握手产物(字段与 v4/v5/v6 对齐)。
...
// AsServerPacketConn 从 CmdUDP 握手结果构造 UDP 数据报适配。UDP 帧格式版本无关 → 复用 snellv6 的适配,
// 只传 v123 的 SS-AEAD 编解码器(SenderSS/ReceiverSS 满足其 chunkEncoder/chunkDecoder 接口)。rbuf 里若有
// 命令 chunk piggyback 的首个 UDP frame,作 initial 先消费。
//
// ★v3 UDP 必须先回 CommandTunnel(0x00)chunk:mihomo 客户端拨号阶段【不】显式 ReadReply(version<4 门控),
// 但 packetConn 首个 pc.Conn.Read 会走 Snell.Read → 内部无条件 ReadReply(s.reply 仍 false),读 1 字节要求
// == CommandTunnel。缺此则我方首帧类型字节(0x04/0x06)被当状态字节 → "command not support",响应被丢。
// 与 snellv45 同(那里客户端拨号阶段就 ReadReply,故也须先回);唯 v6 客户端不 ReadReply,故 v6 不回。
func (r *AcceptResult) AsServerPacketConn() (*snellv6.ServerPacketConn, bool) {
	sc, ok := r.Conn.(*serverConn)
	if !ok {
		return nil, false
	}
	enc, err := sc.send.EncodeChunk([]byte{0x00}) // 首个服务端 chunk:salt + [CommandTunnel]
	if err != nil {
		return nil, false
	}
	...
	return snellv6.NewServerPacketConn(sc.conn, sc.br, sc.send, sc.recv, sc.rbuf), true
}

// Accept 用端口 PSK 跑握手,解出命令 + 返回中继流。不 dial target。
func (s *Server) Accept(client net.Conn) (*AcceptResult, error) {
	br := bufio.NewReader(client)
	recv := NewReceiverSS(s.PSK, s.ChaCha)
	...
	for {
		...
		if cmd != nil {
			...
			sc := &serverConn{conn: client, br: br, send: NewSenderSS(s.PSK, s.ChaCha), recv: recv, rbuf: append([]byte(nil), acc[n:]...)}
