// Package muxcool 实现 Xray 的 Mux.cool 多路复用线协议(帧编解码 + 双端会话),
// 是 NTR 反向代理(bridge/portal)的引擎。
//
// ★协议无关:Mux.cool 帧作为【不透明载荷】跑在任意 NTR 代理流之上 —— 反连隧道用哪个
// 代理(vless/vmess/trojan/ss/snell/socks)由部署决定,muxcool 一概不知、也不需要知道。
// 这正是"不改协议即普适支持反连"的关键:reverse 是【代理契约之上】的一层,不碰协议内部。
// (Xray 的形态 B「VLESS 原生 Rvs 命令」才需要改 VLESS;NTR 走形态 A —— 路由触发、协议无关。)
//
// 本文件只含"帧层"(codec):FrameMetadata 读写 + 数据分块 + port-then-address 地址编解码。
// 会话状态机:server.go(Bridge 侧 ServerWorker)/ client.go(Portal 侧 ClientWorker)。
//
// 与 sing 系的 mux(yamux/smux/h2mux)完全不同、不可复用——那是别的多路复用协议。
// 字节布局对齐 Xray-core common/mux/frame.go(全 big-endian),已对真 Xray 逐字节验穿。
package muxcool
...
// ⚠ 与 NTR addr 系及某些实现的 AddrType(Domain=3/IPv6=4)不同,codec 内部一律用这套。
const (
	atypIPv4   byte = 0x01
...
// framePool 复用帧头编码缓冲(承 NTR 0-alloc 池化原则):写路径每帧不再 make meta/hdr 两块。
// writeMu 已串行化 Worker 的所有写,包级 WriteFrame 各调用取/还各自一块,故池化安全。
var framePool = sync.Pool{New: func() any { b := make([]byte, 0, 576); return &b }} // 512 metaLen 上限 + 前缀余量

// metaReadPool 复用 ReadFrame 的元数据体读缓冲(固定 maxMetaLen;解析后即弃故可池化)。
var metaReadPool = sync.Pool{New: func() any { b := make([]byte, maxMetaLen); return &b }}

// encodeHeader 把 [2B metaLen][元数据体][2B dataLen(仅 Option&Data)] 编码进复用缓冲 dst(不含
// data 段——data 由 WriteFrame 单独写,避免拷贝大 payload)。只写 target 地址块;不写
// source/local/GlobalID(形态 A 语义)。0 alloc(dst 容量够时)。
func encodeHeader(dst []byte, m *FrameMetadata, dataLen int) ([]byte, error) {
	dst = append(dst, 0, 0) // metaLen 占位,末尾回填
	dst = binary.BigEndian.AppendUint16(dst, m.SessionID)
	dst = append(dst, byte(m.Status), byte(m.Option))
	// New,或 (Keep 且 UDP) 时写目标地址块。
	if m.Status == StatusNew || (m.Status == StatusKeep && m.Network == NetworkUDP) {
		dst = append(dst, byte(m.Network))
		var err error
		dst, err = writeAddrPort(dst, m.Port, m.Address)
		if err != nil {
			return nil, err
		}
	}
	metaLen := len(dst) - 2 // 减去 metaLen 前缀本身
	if metaLen > maxMetaLen {
		return nil, ErrMetaTooLong
	}
	binary.BigEndian.PutUint16(dst[0:2], uint16(metaLen))
	if m.Option.Has(OptionData) {
		dst = binary.BigEndian.AppendUint16(dst, uint16(dataLen))
	}
	return dst, nil
}
...
func WriteFrame(w io.Writer, m *FrameMetadata, data []byte) error {
	...
	// [metaLen][meta][dataLen] 拼进池化缓冲一次写出;data 单独写(不拷贝大 payload)。
	hbp := framePool.Get().(*[]byte)
	hdr, err := encodeHeader((*hbp)[:0], m, len(data))
	if err != nil {
		framePool.Put(hbp)
		return err
	}
	_, werr := w.Write(hdr)
	*hbp = hdr[:0] // 保留扩容后的容量供复用
	framePool.Put(hbp)
	if werr != nil {
		return werr
	}
...
func ReadFrame(r io.Reader) (*FrameMetadata, []byte, error) {
	...
	// metaBuf 解析后即弃(unmarshalMeta 把 Address 的 IP/域名 copy 出),故池化复用免每帧 make。
	mbp := metaReadPool.Get().(*[]byte)
	metaBuf := (*mbp)[:metaLen]
	if _, err := io.ReadFull(r, metaBuf); err != nil {
		metaReadPool.Put(mbp)
		return nil, nil, err
	}
